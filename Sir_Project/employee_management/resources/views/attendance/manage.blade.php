...
<style>
    ...
    #attendanceList tr:nth-child(even) {
        background-color: #f2f2f2;
    }

    #attendanceList tr:hover {
...
<script type="text/javascript">
            ...
            if (user.role_name == "ROLE_TEACHER") {
                if (course[i].email == user.email) {
                    html += '<tr class="even">'
                        ...
                        + '<td class="hidden-xs"><button type="button" onclick="viewAtt( \'' + user.email + '\', \'' + course[i].course_code + '\', \'' + course[i].batch_code + '\', \'' + course[i].semester_code + '\')" class="btn btn-primary btn-min-width mr-1 mb-1" data-toggle="modal" data-target="#myModal" id="trigger-btn"">View Attendance Log</button></td>'   // + '<td class="hidden-xs"><button type="button" onclick="initAtt()" class="btn btn-primary btn-min-width mr-1 mb-1">Take Attendance</button></td>'
                        + '</tr>';
                }
            } else {
                html += '<tr class="even">'
    ...
    function viewAtt(userEmail, courseCode, batchCode, semesterCode) {
        console.log(userEmail + courseCode + batchCode + semesterCode);
        $.get("attendance/rpt", {
            ...
            if (user.role_name == "ROLE_TEACHER") {
                if(clsCount>0){
                    $("#modalTitleLabel").text(cls[0].batch_code+ " > " + cls[0].course_code + " > " + cls[0].course_name + " > Faculty Name : " + user.name);
                    if (clsCount > 0) {
                        $("#attCountLabel").text("Detail Attendance Report (Total Lecture : " + clsCount + " )");
                    } else {
                        $("#attCountLabel").text("Detail Attendance Report (Total Lecture : 0)");
                    }
                    ...
                    for (i = 0; i < clsCount; i++) {
                        var clsDate = cls[i].class_date;
                        //thead+= '<th>'+cls[i].classNo+'<small class="text-muted">('+cls[i].classDate+')</small></th>';
                        thead += '<th style="text-align: center;" class="">' + clsDate + '</th>';
                    ...
                    for (i = 0; i < std.length; i++) {
                        ...
                        html += '<tr class="even">'
                            ...
                            /* 	+'<td class="center">'
                                ...
                                +'</td>' */
                            + '<td>' + std[i].roll_no + '</td>'
                            + '<td>' + std[i].student_name + '</td>';

                        var totalPresent = 0;

                        //let j;
                        for (let j = 0; j < att.length; j++) {
                            if (std[i].student_code == att[j].student_code) {
                    ...
                    $('#attendanceList thead').html('');
                    $("#attCountLabel").text('');
                    $("#modalTitleLabel").text( 'Faculty Name : ' + user.name + ' -> No Class taken yet.');
                }
            } else {
                alert(3)
                // $("#modalTitleLabel").text(att[0].student_batch_code+ " > " + att[0].student_course_code + " > " + att[0].student_course_name + " > Faculty Code : " + {{$user->user_code}});
